...
export const getPerfilCompleto = async (req, res) => {
  ...
  try {
    ...
    const [usuarios] = await db.query(
      `SELECT nombre, username, email, foto_perfil, bio, ciudad, departamento, telefono, genero 
       FROM usuarios 
       WHERE email = ?`,
      [email]
    );
    ...
    if (visitor === email) {
      ...
      // Ve perfil de otro usuario (filtrado por preferencias)
      const [rows] = await db.query(
        `
        SELECT 
          p.*,
...
export const insertarUsuario = async (datos) => {
  ...
  return await db.query(
    ...
    (email, nombre, username, telefono, genero, departamento, ciudad, fecha_nacimiento, contrasena, provider) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      email,
      nombre,
