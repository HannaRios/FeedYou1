...
export default function AdminDashboard() {
  ...
  return (
    <div className="flex h-screen bg-[#fcfcfd] overflow-hidden font-sans text-slate-600">
      
      {/* SIDEBAR */}
      <aside className="w-72 h-screen flex flex-col p-8 bg-gradient-to-b from-[#e2e8f0] via-[#fbcfe8] to-[#cbd5e1] border-r border-white/20 shadow-2xl z-50">
        ...
        <nav className="flex-1 space-y-2 overflow-y-auto pr-2">
          {[
            ...
            <button key={item.id} onClick={() => setTabActual(item.id)}
              className={`w-full text-left px-6 py-3.5 rounded-[20px] text-sm transition-all duration-300 ${tabActual === item.id ? 'bg-white text-slate-900 shadow-md font-medium' : 'text-slate-600 hover:bg-white/30'}`}>
              {item.label}
            </button>
          ))}
        </nav>
      ...
      <main className="flex-1 h-screen overflow-y-auto relative bg-[#fcfcfd]">
        <div className="sticky top-0 z-40 bg-[#fcfcfd] px-10 lg:px-14 pt-10 pb-4">
          <header className="flex justify-between items-center bg-white p-6 rounded-[30px] border border-slate-100 shadow-lg">
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-400 font-bold tracking-[0.3em] uppercase mb-1">Panel Operativo</span>
              <h1 className="text-2xl font-semibold text-slate-800 capitalize leading-none">{tabActual === 'inicio' ? 'Resumen General' : tabActual.replace('-', ' ')}</h1>
            </div>
            <div className="flex items-center gap-5">
              <div className="text-right leading-tight">
                <p className="text-sm font-semibold text-slate-800">Admin</p>
                <p className="text-[10px] text-emerald-500 font-black uppercase tracking-tighter">Conectado MySQL</p>
              </div>
              <div className="w-12 h-12 rounded-full border-2 border-purple-100 overflow-hidden shadow-sm bg-slate-50">
                <img src={`${API_URL}/uploads/perfiles/default.png`} alt="Perfil" className="w-full h-full object-cover" />
              </div>
            </div>
          </header>
        </div>

        <div className="px-10 lg:px-14 pb-20 relative z-0">
          ...
          {tabActual === 'usuarios' && (
            <div className="bg-white rounded-[40px] p-10 border border-slate-100 shadow-sm animate-in slide-in-from-bottom-4 duration-700">
              <div className="flex justify-between items-center mb-10">
                <h2 className="text-3xl font-bold text-slate-800 tracking-tight">Gestión de Cuentas</h2>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {usuarios.map((u) => (
                      <tr key={u.email} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="py-6 text-slate-700 font-medium">{u.nombre}</td>
                        <td className="py-6 text-slate-400 font-mono text-xs">{u.email}</td>
                        <td className="py-6 text-center">
                          <button onClick={() => handleAccionUsuario(u.email, u.estado)} className={`px-6 py-2 rounded-full text-[10px] font-bold text-white uppercase shadow-md ${u.estado === 'baneado' ? 'bg-emerald-500' : 'bg-slate-900'}`}>
          ...
          {tabActual === 'denuncias' && (
            <div className="bg-white rounded-[40px] p-10 border border-slate-100 shadow-sm animate-in slide-in-from-bottom-4 duration-700">
              <h2 className="text-3xl font-bold text-slate-800 mb-10">Denuncias Recibidas</h2>
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead>
                    <tr className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">
                      <th className="px-4 pb-4">Denunciante</th>
                      <th className="pb-4">Acusado</th>
                  ...
                  <tbody>
                    {denuncias.map((d) => (
                      <tr key={d.id_denuncia} className="border-b border-slate-50">
                        <td className="py-5 px-4 font-medium">{d.denunciante}</td>
                        <td className="py-5 text-rose-500">{d.acusado}</td>
                        <td className="py-5 text-xs font-bold uppercase">{d.motivo}</td>
                        <td className="py-5 text-[10px] font-bold uppercase">{d.estado_denuncia}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {tabActual === 'Soporte' && (
            <div className="bg-white rounded-[40px] p-20 border border-slate-100 text-center shadow-sm">
              ...
              <p className="text-slate-400 mb-6 italic text-sm">¿Problemas con el servidor o MySQL? Contáctanos.</p>
              <div className="bg-slate-50 py-3 px-6 rounded-full inline-block font-mono text-indigo-600 font-bold">feedyou.hvn@gmail.com</div>
            </div>
          )}
